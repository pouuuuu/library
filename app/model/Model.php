...
class Model {
	...
	public function getPopularResources(int $nbResources): array {
		...
		$resources = array_merge($this->getPopularFilms($nbFilms), $this->getPopularBooks($nbBooks));
	...
	private function getPopularFilms(int $nbFilms): array {
		$sqlFilms = "
        SELECT
            'film' AS resource_type,
            f.idFilm AS id,
        ...
        WHERE f.poster IS NOT NULL AND f.poster != ''
        GROUP BY f.idFilm, f.nbEmprunts
        ORDER BY f.nbEmprunts, rating DESC
        LIMIT :nbFilms
    	";
	...
	private function getPopularBooks(int $nbBooks): array {
		$sqlBooks = "
        SELECT
			'book' AS resource_type,
			l.idLivre AS id,
		...
		WHERE l.couverture IS NOT NULL AND l.couverture != ''
		GROUP BY l.idLivre, l.nbEmprunts
		ORDER BY l.nbEmprunts, rating DESC
		LIMIT :nbBooks
    	";
